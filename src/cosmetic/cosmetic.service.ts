...
export class CosmeticService {
    ...
    async createCape(request, file, data: CreateCapeDTO) {
        ...
        if (isExistentCape)
            throw new ConflictException(
                "Uma capa com esse nome de arquivo já foi criada."
            );
    ...
    async atributeCape(request: Request, data: AtributeCapeDTO) {
        ...
        // @ts-expect-error
        const [user, cape]: { user: User; cape: Cape } = await Promise.all([
            userPromise,
            capePromise
        ]);

        if (!user) throw new NotFoundException("Usuário não encontrado");
        if (!cape) throw new NotFoundException("Capa não encontrada");
    ...
    async getUserCapes(request: Request) {
        if (request["user"].isAdmin) return this.prisma.cape.findMany();
